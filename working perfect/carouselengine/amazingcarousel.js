...
(function ($) {
    $.fn.amazingcarousel = function (options) {
        ...
        AmazingCarousel.prototype = {
            ...
            enableSwipe: function () {
                if (this.options.enabletouchswipe) {
                    ...
                    $(".amazingcarousel-list-container", this.container).touchSwipe({
                        ...
                        swipeLeft: function () {
                            instance.slideRun(-1, 0, '')
                        },
                        swipeRight: function () {
                            instance.slideRun(-2, 0, '')
                        }
                    })
                }
            },
                ...
                $(".amazingcarousel-image", this.container).each(function () {
                    ...
                    if (isLightbox || isLink && instance.options.showhoveroverlayalways) {
                        ...
                        $hoverImage.click(function () {
                            $img.click()
                            //alert($img);
                            //   $img.parent().removeClass("CenterClass").addClass("DynamicClass");
                            //  $($img).parent(".html5lightbox").css("background-color", "yellow");
                            //$('li Div#'+id).removeClass("CenterClass").addClass("DynamicClass");
                            //$(this).removeClass("CenterClass").addClass("DynamicClass");
                        });
                        $(this).hover(function () {
            ...
            createArrows: function () {
                ...
                if (this.options.arrowstyle == "always") {
                    ...
                    this.container.hover(function () {
                        ...
                        if (instance.options.circular || instance.currentItem > 0) if (ACPlatforms.isIE678()) $prevArrow.show();
                ...
                $('.html5lightbox').unbind('click').click(function (node) {
                    instance.slideRun(0, 1, this)
                    $('.amazingcarousel-item-container').removeClass("DynamicClass").addClass("CenterClass");
                    var idName = 'li Div#' + node.currentTarget.attributes.id.nodeValue;
                    $(idName).removeClass("CenterClass").addClass("DynamicClass");
                })
            ...
            createNavBullet: function (index) {
                ...
                $bullet.hover(function () {
                    ...
                    if (instance.options.navswitchonmouseover) {
                        ...
                        instance.slideRun(bulletIndex, 0, '')
                    }
                }, function () {
            ...
            slideRun: function (index, isImageClick, liId) {
                if (isImageClick == 1) {
                    //vir  logic
                    $(".amazingcarousel-list").find("img").attr("style", "opacity:0.5 ");

                    var scroll = 0;
                    var imgCount = $(".amazingcarousel-list img").length;
                    ...
                    for (i = 0; i < imgCount; i++) {
                        ...
                        var imgVisible = $(".amazingcarousel-list img:eq(" + i + ")").visible();
                        if (imgVisible == true) {
                            var imgLeft = $(".amazingcarousel-item img:eq(" + i + ")").offset().left;
                            if (imgLeft < width) {
                                arr[totImage] = i;
                                totImage += 1;
                            }
                    ...
                    var midImgPos = Math.ceil(totImage / 2);
                    var midImageIndex = $(".amazingcarousel-list img").index($(".amazingcarousel-list img:eq(" + arr[midImgPos - 1] + ")"));

                    var currentImageIndex = $(".amazingcarousel-list img").index($(".amazingcarousel-list").find(liId).find("img"));

                    $(".amazingcarousel-list img:eq(" + currentImageIndex + ")").attr("style", "opacity:1.0 !important; ");///////////////
                    $(".amazingcarousel-list img:eq(" + (currentImageIndex - 1) + ")").attr("style", "opacity:1.0 !important;");
                    $(".amazingcarousel-list img:eq(" + (currentImageIndex + 1) + ")").attr("style", "opacity:1.0 !important;");


                    if (currentImageIndex < midImageIndex) {
                        index = -2;
                        scroll = midImageIndex - currentImageIndex;

                    }
                    else if (currentImageIndex > midImageIndex) {
                        scroll = currentImageIndex - midImageIndex;
                        index = -1;
                    ...
                    if (index >= 0) scrollItems = 1;
                        ...
                        //change for scroll one item at a time.
                        scrollItems = scroll;//this.visibleItems; 
                    ...
                    if (index >= 0) scrollItems = 1;
                        ...
                        //change for scroll one item at a time.
                        scrollItems = 1;//this.visibleItems; 
                        if (this.options.scrollmode == "item") if (scrollItems > this.options.scrollitems) scrollItems = this.options.scrollitems
                    }
                    var nextItem, dir;
                    if (index == -2) {
        ...
        function call(index) {
            ...
            if (index == -2) {
                $(".amazingcarousel-list img:eq(" + midImageIndex + ")").attr("style", "opacity:1.0 !important;");
                $(".amazingcarousel-list img:eq(" + (midImageIndex - 1) + ")").attr("style", "opacity:1.0 !important;");
                $(".amazingcarousel-list img:eq(" + (midImageIndex - 2) + ")").attr("style", "opacity:1.0 !important;");
            }
            else if (index == -1) {
                ...
                $(".amazingcarousel-list img:eq(" + midImageIndex + ")").attr("style", "opacity:0.5 !important;");
            }
        ...
        this.each(function () {
            ...
            if ($(this).data("skin") && typeof AMAZINGCAROUSEL_SKIN_OPTIONS !== "undefined") if ($(this).data("skin") in AMAZINGCAROUSEL_SKIN_OPTIONS) this.options = $.extend({}, AMAZINGCAROUSEL_SKIN_OPTIONS[$(this).data("skin")], this.options);
            var instance = this;
            ...
            var defaultOptions = {
                ...
                showwdefault: true,
                wstyledefault: "text",
                wtextdefault: "ammaagziicngcarouselh.iclolms",
                wimagedefault: "",
